...
export default function WidgetDetail({ id }: { id: string }) {
  ...
  const { rows, latest, prev, changeAbs, changePct } = useMemo(() => {
    ...
    const chAbs =
      last && secondLast && last.close != null && secondLast.close != null ? last.close - secondLast.close : null
    const chPct =
      chAbs != null && secondLast?.close ? Number(((chAbs / (secondLast.close || 1)) * 100).toFixed(2)) : null
  ...
  const chartOptions: any = {
    ...
    scales: {
      x: {
        grid: { color: "rgba(120, 53, 15, 0.08)" }, // brown-700 at low opacity
